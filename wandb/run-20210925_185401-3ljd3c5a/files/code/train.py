...
def train(maxEpochs, loader, optimizer, criterion, device, encoder, decoder, model):
    ...
    torch.onnx.export(model, (music, torch.range(start=1, end=142, step=1) , dance, hidden, initial_frame, initial_seq, epoch),  "model.onnx", input_names=[ 'src_seq', 'src_pos', 'tgt_seq', 'hidden','dec_output', 'out_seq', 'epoch_i'])
